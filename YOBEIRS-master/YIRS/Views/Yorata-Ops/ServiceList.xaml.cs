...
namespace YIRS.Views.Yorata_Ops
{
    ...
    public partial class ServiceList : ContentPage
    {
        ...
        private async void GenerateInvoiceButton_Clicked(object sender, EventArgs e)
        {
            ...
            try
            {
                if (button?.CommandParameter is ServicesList service)
                {
                    ...
                    // Navigate to Generate Invoice page with service details
                    await NavigateToGenerateInvoice(service);
                }
            }
            catch (Exception ex)
            {
                LogError("GenerateInvoiceButton_Clicked", ex);
                await DisplayError("Navigation Error",
                    "Unable to proceed to invoice generation. Please try again.");
            }
            finally
            {
                if (button != null)
        ...
        private async Task NavigateToGenerateInvoice(ServicesList service)
        {
            try
            {
                SessionManager.Instance.UpdateActivity();

                // Set static properties for the Generate Invoice page
                GenerateInvoice.SelectedServiceName = service.ServiceName;
                GenerateInvoice.SelectedServiceDescription = service.ServiceDescription ?? "No description available";

                if (decimal.TryParse(service.ServiceAmount, out decimal amount))
                {
                    GenerateInvoice.SelectedServiceAmount = amount;
                }
                else
                {
                ...
                // Set revenue head from MainPage or service name
                GenerateInvoice.RevenueHead = MainPage.CollectionPoint; // Or get from your data source

                await Task.Delay(100);
                await Navigation.PushAsync(new Views.Yorata_Ops.GenerateInvoice());
            }
            catch (Exception ex)
            {
                LogError("NavigateToGenerateInvoice", ex);
                throw;
            }
        }
        ...
        private async Task NavigateToPayment(ServicesList service)
        {
            try
            {
                ...
                // Set the static properties that Payment page expects
                ...
                // Navigate to the Default.Payment page (not Views.Default.Payment)
